...
4. The Thailand guide covers HDP and ESS Thailand together because their participant populations substantially overlap. Lead the ESS section with its unstable history: the Thailand portal normally exposes only the current match, and older match/result URLs can become unavailable when the published match changes. Tell shooters to import results while they remain accessible and keep the imported record associated with their own video archive. Explain both import paths, how one shooter can retain results and videos from both systems in one archive, and the limitations of each source. Do not promise that Shooting Cut can recover an ESS result after the source has removed it. Link to the general import and video-overlay guides instead of duplicating them.
